...
function mailInscripcionEspecialOk($fields)
{
	try {
		$email = $fields['email'];
		...
		<body>
			<div class="container">
				...
				<div class="content">
					...
					<p>Nos complace confirmar que tu inscripción ha sido procesada correctamente.</p>
		...
		// Manejo de errores
		error_log("Error en mailInscripcionEspecialOk: " . $th->getMessage());
		return false;
	}
}
...
$app->post("/mp/notificaciones", function (Request $request, Response $response, array $args) {
	...
	try {
		...
		$mp->setNotificacion($fields)->getResult();
	} catch (\Throwable $th) {
		var_dump($th);
	}
